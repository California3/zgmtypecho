...
class Passport_Widget extends Typecho_Widget
{
    ...
    public function doForgot()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $db = Typecho_Db::get();
            $userW = $db->fetchRow($db->select()->from('table.users')->where('mail = ?', $this->request->mail));
            ...
            /* 生成重置密码地址 */
            $hashString = $userW['name'] . $userW['mail'] . $userW['password'];
            $hashValidate = Typecho_Common::hash($hashString);
            $token = base64_encode($userW['uid'] . '.' . $hashValidate . '.' . $this->options->gmtTime);
            ...
            $phpMailer->addAddress($userW['mail'], $userW['name']);

            $phpMailer->Subject = '密码重置';
            $phpMailer->Body    = '<p>' . $userW['name'] . ' 您好，您申请了重置登录密码</p>'
    ...
    public function doReset()
    {
        ...
        $db = Typecho_Db::get();
        $userW = $db->fetchRow($db->select()->from('table.users')->where('uid = ?', $uid));
